...
{
	LPCTSTR lpszText[] = { "Worker ID", "Password", "Log Save Path" };
	LPCTSTR lpszUserLevelText[] = { "Admin", "Engineer", "Operator" };

	for (int i = 0; i < eUserAddObjectMaxCount; ++i)
		CST_DEFAULT(m_ctrlText[i], IDC_STATIC_USER_ADD_TEXT_1 + i, this, 12, 0, TRUE, ColorLightGray, ColorWhite, lpszText[i]);
...
void CDlgEditUser::OnBnClickedBtnUserAddAdd()
{
	...
	{
		AfxMessageBox("The worker could not be added because the required information is blank.");
	...
	{
		AfxMessageBox("Cannot add same worker ID because it exists");
	...
	strPopupText.Format("Worker ID %s has been successfully added. Channel view by worker is set by default", (LPCSTR)m_strUserID);
