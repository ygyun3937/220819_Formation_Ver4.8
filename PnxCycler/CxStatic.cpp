...
//	wcscpy_s(lf.lfFaceName,strFont);			// Set Font Familly
...
//	ULONG	bytesWriten;
	...
	va_start(args, szFmt);
//	bytesWriten = vswprintf_s(szBuffer, szFmt, args);
...
void CxStatic::DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct)
{
	...
	CXMemDC CXMemDC(pDC, &rect, m_bTransparentBk);

	// PAINT SOLID BACKGROUND IF NO BITMAP
	if ( !m_bBitmap){
		if ( !m_bTransparentBk ){
			if (m_fillmode == Normal)
				CXMemDC.FillRect(&rect, m_pBrush);
			else
				DrawGradientFill(&CXMemDC, &rect, m_fillmode);
		}
	}
	else{// DISPLAY BACKGROUND BITMAP
		DrawBitmap(&CXMemDC, &rect);
	}

	// TEXT RENDERING
	CFont* pOldFont  = CXMemDC.SelectObject( &m_font );
	COLORREF rgbText = CXMemDC.SetTextColor(m_rgbText);
	DrawText(&CXMemDC, &rect, m_strText);

	// Restore DC's State
	CXMemDC.SelectObject(pOldFont);
	CXMemDC.SetTextColor(rgbText);
}
...
void CxStatic::DrawBitmap(CDC* pDCMem, CRect* pRect)
{
	...
	pDCMem->TransparentBlt(rect.left, rect.top, rect.Width(), rect.Height(), 
		&dcMem, 0, 0, m_bmInfo.bmWidth - 1, m_bmInfo.bmHeight -1, m_rgbTransparent);
}
