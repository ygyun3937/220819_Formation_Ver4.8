...
int CGridCtrl::GetRowHeightAll()
{
	int nHeight = 0;
	for(int n=0; n < m_arRowHeights.GetCount(); n++)
		nHeight += m_arRowHeights[n];
	return nHeight;
}
...
CList<int>& CGridCtrl::GetSelectedRows()
{
	CCellRange Selection = GetSelectedCellRange();
	lstSelectedRow.RemoveAll();
	for (int row = Selection.GetMinRow(); row <= Selection.GetMaxRow(); row++)
	{
		if ((GetItemState(row, Selection.GetMinCol()) & GVIS_SELECTED) == GVIS_SELECTED)
			lstSelectedRow.AddTail(row);
	}
...
void CGridCtrl::SetFontData(CString strFontName, int nSize, bool bBold, int nRowHeight)
{
	LOGFONT lf;
