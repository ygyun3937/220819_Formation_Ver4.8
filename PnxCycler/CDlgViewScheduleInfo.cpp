...
{
	LPCTSTR lpszText[eViewScheduleInfoObjectMaxCount] = { "No.", "Type", "Charge Voltage(V)", "Discharge Voltage(V)", "Charge Current(A)", "Discharge Current(A)", "Power(W)", "Resistence(mΩ)", "Temp(°C)" };
	...
	m_listGrade.InsertColumn(1, "No.", LVCFMT_CENTER, 60);
	m_listGrade.InsertColumn(2, "Low Limit", LVCFMT_CENTER, 60);
	m_listGrade.InsertColumn(3, "High Limit", LVCFMT_CENTER, 60);
	m_listGrade.InsertColumn(4, "Code", LVCFMT_CENTER, 60);
	m_listGrade.InsertColumn(5, "Progress", LVCFMT_CENTER, 60);
}
