...
#include <cstdio>
#define MODBUS_LOG(fmt, ...) printf("[ modbuspp ]" fmt, ##__VA_ARGS__)
#else
#define MODBUS_LOG(...) (void)0
#endif
...
public:
	...
	void modbus_set_host_port(std::string host, uint16_t port);
 ...
 * .
 * 
 * METHOD_NAME : modbus_set_host_port
 * METHOD_TYPE : -
 * DESCRIPTION : ACW
 */
inline void modbus::modbus_set_host_port(std::string host, uint16_t port)
{
	HOST = host;
	PORT = port;
	_slaveid = 1;
...
inline bool modbus::modbus_connect()
{
    ...
    {
        MODBUS_LOG("Missing Host and Port");
        return false;
    }
    else
    {
        MODBUS_LOG("Found Proper Host %s and Port %d", HOST.c_str(), PORT);
    }
    ...
    {
        MODBUS_LOG("Error Opening Socket");
#ifdef _WIN32
        WSACleanup();
#endif
        return false;
    ...
    {
        MODBUS_LOG("Socket Opened Successfully");
    }
