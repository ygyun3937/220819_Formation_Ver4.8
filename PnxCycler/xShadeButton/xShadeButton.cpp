 ...
 * - fixed second tooltip initialization bug (by Jesper Kinn?)
...
/////////////////////////////////////////////////////////////////////////////
void CxShadeButton::SetIcon(UINT nIcon, UINT nIconAlign, int nXMargin, UINT nIconDown, UINT nIconHighLight)
{
	if (m_IconDown!=m_Icon && m_IconDown) DestroyIcon(m_IconDown);
	if (m_IconHighLight!=m_Icon && m_IconHighLight) DestroyIcon(m_IconHighLight);
	if (m_Icon) DestroyIcon(m_Icon);
	...
	if (m_Icon){							// if success...
		...
		case BS_LEFT:
			m_IconAlign=BS_LEFT;
			m_IconBox.OffsetRect(m_FocusRectMargin+nXMargin,
								 max(0, (long)(y/2 - iinfo.yHotspot)));
			break;
		default:
...
bool CxShadeButton::SetFont(CString sFontName,long lSize, long lWeight, BYTE bItalic, BYTE bUnderline)
{
	...
	if (m_pLF){
		...
		m_pLF->lfQuality = CLEARTYPE_QUALITY;
		m_Font.DeleteObject();
...
void CxShadeButton::OnTimer(UINT_PTR nIDEvent)
{
	if (m_bPLCType)
	{
		KillTimer(nIDEvent);
		...
		BYTE highlight1 = 10;
		BYTE coloring1 = 90;
		COLORREF color1 = ColorRed; 
		SetShade(SHS_HSHADE, granularity1, highlight1, coloring1, color1);
		Invalidate(FALSE);
	}
...
BOOL CxShadeButton::PreTranslateMessage(MSG* pMsg)
{
	if (m_bPLCType)
	{
...
void CxShadeButton::SetButtonLampSet(COLORREF color)
{
	if (m_LampColor != color)
	{
		m_LampColor = color;
