...
void CGroupListCtrl::DrawItems(int nGroupIndex, CDC *pDC, sGroupInfo* lpGroupInfo, CRect rcGroup, bool bDrawText, bool bDrawBattery)
{
	if (nullptr == lpGroupInfo)
		return;
...
void CGroupListCtrl::SetSubItemColor(int nGroupIndex, int nItemIndex, int nSubItemIndex, COLORREF subItemColor, BOOL bUseColor, int nBattery)
{
	if (nGroupIndex < 0 || nGroupIndex >= m_arrGroupInfo.GetCount())
		return;
...
void	CGroupListCtrl::SetItemSelected(int nGroupIndex, int nItemIndex, BOOL bFlag)
{
	if (nGroupIndex < 0 || nGroupIndex >= m_arrGroupInfo.GetCount())
		return;
...
void CGroupListCtrl::EditColumn(CString strText, int nWidth, eTextFormat eFormat, int dIndex)
{
	sColumnInfo *pInfo = NULL;
...
int CGroupListCtrl::GetColumnHeight()
{
...
void	CGroupListCtrl::SetItemFlag(int nGroupIndex, int nItemIndex, bool bReserveCommand, bool bMultiSchedule)
{
					...
					{					
						...
						if (rcItem.PtInRect(point) == TRUE)
						{
							...
							if (!m_bClickCtrl && !m_bClickShift)
							{
								...
								if (pWnd)
									::SendMessage(pWnd->GetSafeHwnd(), WM_GROUP_SELECT, (WPARAM)strSelectedIndex.operator LPCTSTR(), nSubItemIndex);
							}
						...
						if (m_bClickShift)
						{
							if ((nItemIndex >= m_nStartIndex && nItemIndex <= m_nEndIndex)
								||(nItemIndex <= m_nStartIndex && nItemIndex >= m_nEndIndex))// 아래에서 위로 선택할수 있도록 조건 추가. 2020-12-28
							{
								pItemInfo->bSelected = TRUE;
							}
						}
		...
		if (pWnd)
		{
			if (m_bClickLButton && !m_bClickShift && !m_bClickCtrl)
			{
				...
				for (int i = 0; i < m_arrColumnInfo.GetCount(); i++)
				{
					...
					{
						m_nLButtonColomn = i;
						::SendMessage(pWnd->GetSafeHwnd(), WM_GROUP_COLUMN_LCLICK, (WPARAM)m_nLButtonColomn, (LPARAM)GetDlgCtrlID());
						break;
					}
				}
			}
			else
				::SendMessage(pWnd->GetSafeHwnd(), WM_GROUP_SELECT, (WPARAM)strSelectedIndex.operator LPCTSTR(), m_bUseClickEvent);
		}
	}	
...
int		CGroupListCtrl::GetSelectedItemCount()
{
	int nItemCount = 0;
...
void	CGroupListCtrl::OnRButtonDown(UINT nFlags, CPoint point)
{
	...
	*/
	...
	if (nSelectedIndex >= 0)
	{
		...
		if (pWnd)
			::SendMessage(pWnd->GetSafeHwnd(), WM_GROUP_RCLICK, nSelectedIndex, 0);
	}	
...
void	CGroupListCtrl::SetFontSizeTitle(int nFontSize)
{
...
void CGroupListCtrl::OnMouseHover(UINT nFlags, CPoint point)
{
	TRACKMOUSEEVENT tme;
	...
	if (nColumn == -1)
	{
		if (m_nOverColumn != -1)
		{
			...
			{
				if (pWnd)
					::SendMessage(pWnd->GetSafeHwnd(), WM_GROUP_COLUMN_LEAVE, (WPARAM)m_nOverColumn, (LPARAM)GetDlgCtrlID());
			}
			m_nOverColumn = -1;
		}
	}
	else if (m_nOverColumn != nColumn)
	{
		...
		{
			if (pWnd)
				::SendMessage(pWnd->GetSafeHwnd(), WM_GROUP_COLUMN_OVER, (WPARAM)m_nOverColumn, (LPARAM)GetDlgCtrlID());
		}
	}
...
void CGroupListCtrl::OnMouseLeave()
{
	if (m_nOverColumn != -1)
	{
		CWnd* pWnd = GetParent();
	...
	if (pMsg->message == WM_KEYDOWN)
	{
		if (pMsg->wParam == VK_RETURN)
		{
			...
			if (m_pEditCtrl != NULL && m_nCurEditGroupIndex != -1 && m_nCurEditItemIndex != -1 && m_nCurEditSubItemIndex != -1)
			{
				...
				if (pWnd)
					::SendMessage(pWnd->GetSafeHwnd(), WM_GROUP_EDIT_CHANGE, (WPARAM)strText.operator LPCTSTR(), m_nCurEditItemIndex);
...
void CGroupListCtrl::SetColumnGradationColor(COLORREF colorColumnGradaition)
{
	m_colorGradation = colorColumnGradaition;
}

void CGroupListCtrl::SetUseClickEvent(BOOL bFlag)
{
	m_bUseClickEvent = bFlag;
}

void	 CGroupListCtrl::RemoveAllItems_WithScrollReset()
{
	SettingScrollBar();
	RemoveAllItems();
}

void	CGroupListCtrl::SetBatteryStatusIndex(int nIndex)
{
